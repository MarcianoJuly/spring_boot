...
public class ListController {
    ...
    @PutMapping("/edit/{CPF}")
    @CrossOrigin(origins = "http://localhost:4200")
    public ResponseEntity<DataClient> put(@PathVariable String CPF, @RequestBody DataClient dados) {
        return dataClientRepositorio.findById(CPF)
        .map(recordFound -> {recordFound = dados;
                            DataClient update = dataClientRepositorio.save(recordFound);
